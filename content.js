...
// SVG icon helpers — must be defined before BADGE_STATES
function svgPause() {
...
function injectBadgeStyles() {
  ...
  style.textContent = `
    ...
    #${BADGE_ID}.sdb-waiting .sdb-dot {
      animation: sdb-pulse 1.8s ease-in-out infinite;
    }
  `;
...
function teardown() {
  ...
  // Dim the badge to signal the script is no longer active
  const badge = document.getElementById(BADGE_ID);
  if (badge) badge.style.opacity = "0";
}

// Detect extension reload/update and tear down gracefully
try {
